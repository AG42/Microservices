...
namespace PurchaseOrderBySupplier.Common
{
    public class ConfigReader
    {
        ...
        public Dictionary<string, string> GetDatabaseTableName(string companyCode, string parentTableNameKey)
        {
            if (!_readFromDatabase)
            {
                ...
                {
                    {Constants.TableNameKey, ReadConfig($"{Constants.DatalakeTableNameKey}_{companyCode.ToLower()}")},
                    {Constants.ColumnNameKey, ReadConfig($"{Constants.DatalakeColumnNameKey}_{companyCode.ToLower()}")}
                };
                return dicTableName;
            }
            string configurationDbConnectionString = ReadConfig("ConfigurationDbConnectionString");
        ...
        public Dictionary<string, string> GetDatabaseTableName(string companyCode, string parentTableNameKey, string tableNameKey, string columnNameKey)
        {
            if (!_readFromDatabase)
            {
                var dicTableName = new Dictionary<string, string>
                {
                    {Constants.TableNameKey, ReadConfig($"{tableNameKey}_{companyCode.ToLower()}")},
                    {Constants.ColumnNameKey, ReadConfig($"{columnNameKey}_{companyCode.ToLower()}")}
                };
                return dicTableName;
            }
            string configurationDbConnectionString = ReadConfig("ConfigurationDbConnectionString");
