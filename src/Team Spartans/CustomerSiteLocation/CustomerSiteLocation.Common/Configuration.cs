...
namespace CustomerSiteLocation.Common
{
    public class Configuration
    {
        ...
        public string GetDatalakeTableName(string serviceName, string environment, string companyCode, string tableNameKey, string parentCompanyCode)
        {
            try
            {
                var parameterCollection = new List<SqlParameter>
               {
                ...
                if (dataSet.Tables.Count > 0 && dataSet.Tables[0].Rows.Count > 0)
                    return $"{dataSet.Tables[0].Rows[0]["DatabaseName"]}.{dataSet.Tables[0].Rows[0]["TableName"]}";
                throw new Exception(
